...
**stash it, do not `git checkout --` it.**
...
- Write a message that says what the work does **and** what blocked it. That is
  what makes the entry findable and re-runnable later.
- `git stash list` and `git stash show -p stash@{n}` before assuming an entry is
  stale. Restore with `git stash pop`, or `apply` to keep the entry.
...
- Say in your summary that the work is stashed and under what message, so it is
  not silently lost.

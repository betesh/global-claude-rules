...
- Check the repo for an existing single-file runner or filter argument before
  inventing one — most suites have a name filter or accept a path. If that
  runner is a thin wrapper around an installed package, see
  `install-before-filesystem-search.md` for where the filter argument is
  actually documented.
